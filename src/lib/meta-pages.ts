...
async function fetchAllPages(
  ...
  const pages: FbPage[] = [];
  let url: string | null = `https://graph.facebook.com/v19.0/me/accounts?fields=${encodeURIComponent(
    fields
  ...
  while (url) {
    ...
    if (pages.length >= 500) break; // guard against a runaway paging cursor
  }
  return pages;
}
